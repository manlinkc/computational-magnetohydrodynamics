...
/*
 File Description: 2D MHD MUSCL-Hancock Method with HLLC, with divergence cleaning 
 */
...
int main(void){
    ...
    std::function<void(std::vector<std::vector<std::array<double, 9> > >&,std::vector<std::vector<std::array<double, 2> > >&)> initialiseFn;
    switch(testnum){
        case 4:
    ...
    std::function<void(std::vector<std::vector<std::array<double, 9> > >&,int &, int &)> boundaryconditionsFn;
    switch(bconditions){
        case 1:
	...
	std::vector < std::vector < std::array<double, 9> > > UP;
	UP.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	...
	for (int i = 2; i < int(UP.size()-2); i++){
			...
			for (int j = 2; j < int(UP[1].size()-2); j++){
                ...
                output_initial << x << " " << y << " " << UP[i][j][0] << " " << UP[i][j][1] << " " << UP[i][j][2] << " " << UP[i][j][3] << " " << UP[i][j][4] << " " << UP[i][j][5] << "  " << UP[i][j][6] << " " << UP[i][j][7] << " " << UP[i][j][8]<< std::endl;
			}
    ...
    std::ofstream divfile(test+"_nablaB.txt");
	
	//---------------------INITIALIZE VECTORS---------------------------
	//------------------------------------------------------------------
	// Define a vector to store conservative variables
	// Needs to be the same size as UP
	std::vector < std::vector < std::array<double, 9> > > UC;
	UC.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store the UC at time i+1 flux values
	std::vector < std::vector < std::array<double, 9> > > UCiplus1;
	UCiplus1.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store the UCline after x update
	std::vector < std::vector < std::array<double, 9> > > UCline;
	UCline.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store SLIC flux variables for f flux
	std::vector < std::vector < std::array<double, 9> > > SLICflux_f;
	SLICflux_f.resize(xCells + 1, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store SLIC flux variables for g flux
	std::vector < std::vector < std::array<double, 9> > > SLICflux_g;
	SLICflux_g.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 1) );
	
    // Define a vector to store SLIC flux variables for f flux
    std::vector < std::vector < std::array<double, 9> > > HLLCflux_f;
    HLLCflux_f.resize(xCells + 1, std::vector< std::array<double,9 > >(yCells + 4) );
    
    // Define a vector to store SLIC flux variables for g flux
    std::vector < std::vector < std::array<double, 9> > > HLLCflux_g;
    HLLCflux_g.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 1) );
    
	// Define a vector to store Uleft quantity
	std::vector < std::vector < std::array<double, 9> > > Uleft;
	Uleft.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Ulineleft quantity
	std::vector < std::vector < std::array<double, 9> > > Ulineleft;
	Ulineleft.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Uright quantity
	std::vector < std::vector < std::array<double, 9> > > Uright;
	Uright.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Ulineright quantity
	std::vector < std::vector < std::array<double, 9> > > Ulineright;
	Ulineright.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Uleft nplushalf quantity
	std::vector < std::vector < std::array<double, 9> > > Uleft_nph;
	Uleft_nph.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Ulineleft nplushalf quantity
	std::vector < std::vector < std::array<double, 9> > > Ulineleft_nph;
	Ulineleft_nph.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Uright nplushalf quantity
	std::vector < std::vector < std::array<double, 9> > > Uright_nph;
	Uright_nph.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
	
	// Define a vector to store Ulineright nplushalf quantity
	std::vector < std::vector < std::array<double, 9> > > Ulineright_nph;
	Ulineright_nph.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) );
    std::vector < std::vector < std::array<double, 9> > > source;
    source.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) ); //Source terms
    std::vector < std::vector < std::array<double, 9> > > UCsource;
    UCsource.resize(xCells + 4, std::vector< std::array<double,9 > >(yCells + 4) ); //ODE source term update
	...
	do{
        ...
        for(int i=1; i< int(UC.size())-1; i++){
            for(int j=0; j< int(UC[1].size()); j++){
                for(int var=0; var<9; var++){
                    if (var==5){ //Keep Bx constant
                        Uleft[i][j][var] = UC[i][j][var];
                        Uright[i][j][var] = UC[i][j][var];
                    }
                    else if (var==8){ //Keep Psi constant
                        Uleft[i][j][var] = UC[i][j][var];
                        Uright[i][j][var] = UC[i][j][var];
                    }
                    else{
                        ...
                        //std::cout << "Code reaches here" <<std::endl;
		...
		for (int i=0; i<int(Uleft.size()); i++){
			for (int j=0; j<int(Uleft[1].size()); j++){
				...
				std::array<double, 9> f_Uleft;
				std::array<double, 9> f_Uright;
				...
				for (int var=0; var<9; var++){
                    if (var==5){ //Keep Bx constant
                        Uleft_nph[i][j][var] = Uleft[i][j][var];
                        Uright_nph[i][j][var] = Uright[i][j][var];
                    }
                    else if (var==8){ //Keep Psi constant
                        Uleft_nph[i][j][var] = Uleft[i][j][var];
                        Uright_nph[i][j][var] = Uright[i][j][var];
        ...
        for (int i=0; i< int(HLLCflux_f.size()); i++){
            for (int j=0; j< int(HLLCflux_f[1].size()); j++){
                ...
                //Calculate Btilde;
                double Bxtilde = getBxtilde(Uright_nph[i+1][j], Uleft_nph[i+2][j], ch);
                std::array<double,2> BPsi_x = getDCflux_x(Uright_nph[i+1][j], Uleft_nph[i+2][j], ch);
    
                std::array<double,9> uLx = Uright_nph[i+1][j];
                std::array<double,9> uRx = Uleft_nph[i+2][j];
                uLx[5] = Bxtilde;
                uRx[5] = Bxtilde;
                
                HLLCflux_f[i][j] = getHLLCflux_xdirection(uLx, uRx,gamma, BPsi_x);
                //SLICflux_f[i][j] = getFORCEflux_xdirection(uLx, uRx,dx,dt,gamma, BPsi_x);
                 }
        }
		...
		//-------------------------------------------------------------------
        // Get ulineleft
        for(int j=1; j< int(UCline[1].size())-1; j++){
            for(int i=0; i< int(UCline.size()); i++){
                for(int var=0; var<9; var++){
                    if(var == 6){
                        Ulineleft[i][j][var] = UCline[i][j][var];
                        Ulineright[i][j][var] = UCline[i][j][var];
                    }
                    else if(var == 8){
                        Ulineleft[i][j][var] = UCline[i][j][var];
                        Ulineright[i][j][var] = UCline[i][j][var];
		...
		for (int i=0; i<int(Ulineleft.size()); i++){
			for (int j=0; j<int(Ulineleft[1].size()); j++){
				...
				//Get euler flux of ulineleft and ulineright
				std::array<double, 9> f_Ulineleft;
				std::array<double, 9> f_Ulineright;
				...
				for (int var=0; var<9; var++){
                    if (var == 6){
                        Ulineleft_nph[i][j][var] = Ulineleft[i][j][var];
                        Ulineright_nph[i][j][var] = Ulineright[i][j][var];
                    }
                    else if(var == 8){
                        Ulineleft_nph[i][j][var] = Ulineleft[i][j][var];
                        Ulineright_nph[i][j][var] = Ulineright[i][j][var];
        ...
        for (int i=0; i< int(HLLCflux_g.size()); i++){
            for (int j=0; j< int(HLLCflux_g[1].size()); j++){
                ...
                //Calculate Btilde;
                double Bytilde = getBytilde(Ulineright_nph[i][j+1], Ulineleft_nph[i][j+2], ch);
                std::array<double,2> BPsi_y = getDCflux_y(Ulineright_nph[i][j+1], Ulineleft_nph[i][j+2], ch);
    
                std::array<double,9> uLy = Ulineright_nph[i][j+1];
                std::array<double,9> uRy = Ulineleft_nph[i][j+2];
                uLy[6] = Bytilde;
                uRy[6] = Bytilde;
                
                HLLCflux_g[i][j] = getHLLCflux_ydirection(uLy, uRy,gamma, BPsi_y);
                //SLICflux_g[i][j] = getFORCEflux_ydirection(uLy, uRy,dy,dt,gamma, BPsi_y);
        ...
        for (int i=0; i<int(UCiplus1.size()); i++){
            //double x = x0 + (i-1.5)*dx;
            for (int j=0; j<int(UCiplus1[1].size()); j++){
                for (int k=0; k<8; k++){
                    source[i][j][k] = 0.0;
                }
                source[i][j][9] = -1.0*cratio*ch*UCiplus1[i][j][9];
            }
        }
        
        //Update the solution using first-order accurate euler method (see ACM lecture 3, slide 13)
        for (int i=2; i<int(UCsource.size())-2; i++){
            for (int j=0; j<int(UCsource[1].size()); j++){
                for (int k =0; k <9; k++){
                    UCsource[i][j][k] = UCiplus1[i][j][k] + dt * source[i][j][k];
                }
            }
        }
        boundaryconditionsFn(UCsource,xCells, yCells);
        ...
        if( t>tcounter ){
            ...
            for (int i = 2; i < int(UP.size()-2); i++){
                    for (int j = 2; j < int(UP[1].size()-2); j++){
                        double nablaB = ((UP[i+1][j][5] - UP[i-1][j][5])/(2.0*dx)) + ((UP[i][j+1][6] - UP[i][j-1][6])/(2.0*dy));
                        maxnabla = std::max(maxnabla, fabs(nablaB));
                        L1error = L1error + fabs(nablaB);
                    }
            }
            
            L1error = L1error*dx*dy;
            //std::cout << "t = " << t << std::endl;
            //std::cout << "L1error = " << L1error <<std::endl;
            //std::cout << "Max div B = " << maxnabla <<std::endl;
            
            divfile << t << " " << L1error << " " << maxnabla << std::endl;
            tcounter = tcounter + 1.0;
        }
	}while (t < tstop);
	...
	for (int i = 2; i < int(UP.size()-2); i++){
			...
			for (int j = 2; j < int(UP[1].size()-2); j++){
				 ...
				 output_final << x << " " << y << " " << UP[i][j][0] << " " << UP[i][j][1] << " " << UP[i][j][2] << " " << UP[i][j][3] << " " << UP[i][j][4] << " " << UP[i][j][5] << " " << UP[i][j][6] << " " << UP[i][j][7] << " " << ratio << " " << UP[i][j][8] << std::endl;
			}
			//Add extra blank line
